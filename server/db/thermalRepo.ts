 ...
 * Returns null when no local path can be derived (never invent a clip-id folder).
 */
...
/** Losing buyer / second session — webhook should ACK (not 500-retry). */
export class FulfillmentConflictError extends Error {
  readonly code = 'fulfillment_conflict' as const
...
export function isFulfillmentConflictError(err: unknown): err is FulfillmentConflictError {
  return err instanceof FulfillmentConflictError
}
    ...
    try {
      ...
      if (!canClaimClip(row, input.stripeCheckoutSessionId)) {
        throw new FulfillmentConflictError(
          `Clip ${input.clipId} already claimed by another checkout session`,
        )
      }
      await client.query(
        `UPDATE clips
 ...
 * checkout session replaying; reject a second buyer session on an active row.
 */
export function canActivateRetainer(row: {
  status: string
  stripe_checkout_session_id?: string | null
}, sessionId: string): boolean {
  ...
  await withClient(async (client) => {
    ...
    try {
      ...
      if (!canActivateRetainer(row, input.stripeCheckoutSessionId)) {
        throw new FulfillmentConflictError(
          `Retainer ${input.retainerId} already activated by another checkout session`,
        )
      }
      await client.query(
        `UPDATE retainers
