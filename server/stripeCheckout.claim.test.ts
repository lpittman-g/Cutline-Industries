...
describe('stripe lost-claim race handling', () => {
  ...
  it('confirmCheckoutSession returns clip_already_claimed with refund outcome', () => {
    ...
    const body = src.slice(start, next > start ? next : start + 1400)
    assert.match(body, /status:\s*'clip_already_claimed'/)
    assert.match(body, /handleLostClaimRace/)
    assert.match(body, /saleStatus:\s*refund\.saleStatus/)
    assert.match(body, /refunded:\s*refund\.refunded/)
  })
  ...
  it('checkout API routes map claim conflicts to HTTP 409 with saleStatus on confirm', () => {
    assert.match(apiSrc, /status\(409\)/)
    assert.match(apiSrc, /clip_already_claimed/)
    assert.match(apiSrc, /ClipAlreadyClaimedError/)
    assert.match(apiSrc, /saleStatus:\s*result\.saleStatus/)
    assert.match(apiSrc, /refunded:\s*result\.refunded/)
  })
})
